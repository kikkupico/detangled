...
export default function EdgePane({ nodes, direction, labelFn, onSelect }: Props) {
  ...
  const [svgHeight, setSvgHeight] = useState(window.innerWidth >= 768 ? 130 : 100);
  ...
  useEffect(() => {
    const updateDimensions = () => {
      ...
      setSvgHeight(window.innerWidth >= 768 ? 130 : 100);
    };
      ...
      {nodes.length > 0 && (
        ...
        >
          {nodes.map((node, i) => {
            ...
            const labelY = direction === "up" ? circleCY - circleR - 6 : circleCY + circleR + 16;
